...
from pr2.gui import pushToTalk_ui
import basewindow

class MainWindow(QMainWindow, pushToTalk_ui.Ui_MainWindow):
    ...
    def __init__(self):
        ...
        self.faceTableButton.clicked.connect(self.faceTable)
    ...
    def p2tButtonCallback(self, pressed):
        ...
        if self.mode == 'Command':
            ...
            self.log("Listening for speech and executing...")
    ...
    def SLUCommandParserStatusCallback(self, msg):
        if self.inProcess:
            self.log("Command execution complete\n")
            self.p2tButton.setEnabled(True)
            self.p2tButton.setText("Push To Talk")
            self.log("Ready for next message")
    ...
    def pr2CommanderExceptionCallback(self, msg):
        if self.inProcess:
            self.log("ERROR: Could not process nl_input command in pr2_commander:\n")
            self.log(msg["data"])
            self.p2tButton.setEnabled(True)
            self.p2tButton.setText("Push To Talk")
            self.log("Ready for next message")
    ...
    def log(self, string):
        self.statusBox.setPlainText(str(self.statusBox.toPlainText()) + "\n" + string)

    def faceTable(self):
        self.log("--------------------")
        self.log("Face table (command Mode)\n")
        self.mode = "Command"
    ...
    def wordLearningMode(self):
        self.log("--------------------")
        self.log("Word Learning Mode\n")
